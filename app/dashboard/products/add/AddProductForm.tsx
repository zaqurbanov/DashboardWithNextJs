...
import { MdArrowBack, MdShuffle, MdAddBox } from "react-icons/md";
import { addProduct, ProductFormState } from "./actions";

function randomImage() {
  const id = Math.floor(Math.random() * 200) + 60;
  return { id, url: `https://picsum.photos/id/${id}/400/300` };
}
    ...
    <form action={formAction} className="animate-fade-up" style={{ animationFillMode: "both" }}>
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="neu-flat rounded-2xl p-8 flex flex-col items-center gap-5 lg:w-72 shrink-0">
          ...
          <p className="font-bold text-sm text-center truncate max-w-[200px]">
            {title || "New Product"}
          </p>
          <p className="text-xs text-center" style={{ color: "#aaa" }}>
            Auto-assigned from Picsum Photos
          </p>
        </div>
